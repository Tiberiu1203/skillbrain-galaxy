...
const carti = [
    ...
    {titlu : "Crimă și pedeapsă" , autor : "Fyodor Dostoievski" , esteCitita : false},
    {titlu : "Don Quijote" , autor : "Miguel de Cervantes" , esteCitita : true}
];

function getCartiCitite (lista) {
    lista.forEach(carte => {
        console.log(`"${carte.titlu}" de ${carte.autor}`);

        if(carte.esteCitita){
            console.log(`Aceasta carte: "${carte.titlu}" de ${carte.autor} este citita.`)
        } else {
            console.log(`Aceasta carte: "${carte.titlu}" de ${carte.autor} trebuie citita.`)
        }
    })
}

getCartiCitite(carti)